...
## Часть 4. Практические задания (30 минут)

### Задание 4.1. Конфигурация и приоритеты

Создайте три конфигурационных файла с разными значениями `forks` и определите, какой из них будет использован:

```bash
# Создание глобального (системного) конфига с sudo
...
# Создание локального конфига в рабочей директории
cat > ~/ansible-lab/ansible.cfg << 'EOF'
[defaults]
forks = 5
EOF

# Определение используемого файла
cd ~/ansible-lab
ansible --version | grep "config file"

# Проверка значения forks
ansible-config dump | grep DEFAULT_FORKS
```

**Вопрос:** Какой файл конфигурации использовался и почему?

### Задание 4.2. Создание сложного инвентаря

Создайте инвентарь со следующей структурой:

```bash
nano ~/ansible-lab/inventories/complex/hosts.ini
```

Требования:
1. Группа `load_balancers` с хостами `lb01`, `lb02`
2. Группа `app_servers` с хостами `app01`, `app02`, `app03`
3. Группа `cache_servers` с хостами `redis01`, `redis02`
4. Группа `production`, объединяющая все вышеперечисленные
5. Переменная `environment=prod` для группы `production`
6. Переменная `app_version=2.1` для `app_servers`

Проверьте созданный инвентарь:
```bash
ansible-inventory -i inventories/complex/hosts.ini --graph
ansible production -i inventories/complex/hosts.ini --list-hosts
```

### Задание 4.3. Шаблоны хостов

Используя созданный инвентарь, выполните следующие операции:

```bash
# 1. Запустить ping на всех app_servers
ansible app_servers -i inventories/complex/hosts.ini -m ping

# 2. Запустить команду hostname на lb01 и app01
ansible lb01:app01 -i inventories/complex/hosts.ini -m command -a "hostname"

# 3. Запустить на всех, кроме cache_servers
ansible all:!cache_servers -i inventories/complex/hosts.ini --list-hosts

# 4. Запустить на пересечении production и app_servers
ansible production:&app_servers -i inventories/complex/hosts.ini --list-hosts

# 5. Запустить на первых двух app_servers
ansible app_servers[0:2] -i inventories/complex/hosts.ini --list-hosts
```

### Задание 4.4. Продвинутые шаблоны

```bash
# Создайте инвентарь с географической структурой
cat > ~/ansible-lab/inventories/geo-hosts.ini << 'EOF'
[us-east]
web-ny01
web-ny02
db-ny01
...
# Выполните команды:
# 1. Все веб-серверы из americas
ansible web_all:&americas -i inventories/geo-hosts.ini --list-hosts

# 2. Все серверы, кроме тех, что в americas
ansible all:!americas -i inventories/geo-hosts.ini --list-hosts

# 3. Все датабазы из americas и emea
ansible 'db-*' -i inventories/geo-hosts.ini --list-hosts
```
...
| Проблема | Решение |
|----------|---------|
| `Host key verification failed` | Добавьте `host_key_checking = False` в `ansible.cfg` |
| `Permission denied` | Проверьте sudo настройки на managed nodes |
| `Python not found` | Установите python3: `sudo apt install python3` |
| `No hosts matched` | Проверьте синтаксис шаблона и имена в инвентаре |
| `Config file not found` | Убедитесь, что файл существует и имеет правильный синтаксис |
